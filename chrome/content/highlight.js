...
function highlightLanguage(block, language) {
  ...
  var container = initHighlighting.document.createElement('div');
...
function highlightAuto(block) {
  ...
  if(result) {
	...
	{
		container.firstChild.className = block.className
		var environment = block.parentNode;
		environment.replaceChild(container.firstChild, block);
	}
  }//if
}//highlightAuto
...
function initHighlighting() {
  ...
  var pres = initHighlighting.document.getElementsByTagName('pre');
  for (var i = 0; i < pres.length; i++) {
    if (pres[i].firstChild && pres[i].firstChild.nodeName == 'CODE')
      initHighlight(pres[i].firstChild);
	else
	  if (initHighlighting.doOnlyPres) // && pres[i].parentNode.nodeName != 'BODY'
	    initHighlight(pres[i]);
  }//for
}//initHighlighting

function injectScripts(languages) {
  var scripts = initHighlighting.document.getElementsByTagName('SCRIPT');
...
initHighlighting.document = document